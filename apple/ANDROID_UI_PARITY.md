...
| --- | --- | --- |
| Main Screen | Incident / operational period header, restriction status, aircraft table, confirmation flow | Implemented. The Apple table includes BT4, BT5, WiFi, NaN, R2C, Total, duration, and RTT columns. |
...
| Map controls | Layer, Predictive Head, Download Map, Map Folders, Map Management | Implemented in the same order. |
...
- Android Settings exposes minimum movement distance, new-track delay, bridge
  check distance, audio alarm volume/test, maximum idle time, editable proximity
  spacing, and standalone R2C coordination. Apple currently receives only part
...
- iOS applications do not provide a programmatic `Quit` command.
